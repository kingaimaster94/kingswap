...
export const useBnbPrices = (): BnbPrices | undefined => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const { bnbPrices, error: fetchError } = await fetchBnbPrices(block24.number, block48.number, blockWeek.number); // block48.number, blockWeek.number
      // const { bnbPrices, error: fetchError } = await fetchBnbPrices(13461, 13461, 13461);
      if (fetchError) {
        setError(true)
      } else {
        setPrices(bnbPrices)
